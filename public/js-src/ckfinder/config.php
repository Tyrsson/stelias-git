...
function CheckAuthentication()
{
    ...
    if($auth->hasIdentity())
    {
        // User must be at least a moderator and have the media.files.upload privilege to upload
        if($acl->isAllowed('mod', 'media.files', 'media.manage')) {
            return true;
        } else {
            return false;
        }
...
*/
// $baseUrl = '/ckfinder/userfiles/'; <- default
$baseUrl = '/modules/';
...
*/
//$baseDir = resolveUrl($baseUrl);
$baseDir = $_SERVER['DOCUMENT_ROOT'] . DIRECTORY_SEPARATOR . 'modules' . DIRECTORY_SEPARATOR; 
...
Example: 'maxSize' => "8M",
*/
$config['DefaultResourceTypes'] = 'Media,Pages,Slider';

$config['ResourceType'][] = Array(
		'name' => 'Media',				// Single quotes not allowed
		'url' => $baseUrl . 'media/images',
		'directory' => $baseDir . 'media/images',
		'maxSize' => "20M",
...
$config['ResourceType'][] = Array(
		'name' => 'Slider',
		'url' => $baseUrl . 'slider/images',
		'directory' => $baseDir . 'slider/images',
		'maxSize' => "2M",
...
$config['ResourceType'][] = Array(
		'name' => 'Pages',
		'url' => $baseUrl . 'pages/images',
		'directory' => $baseDir . 'pages/images',
		'maxSize' => "20M",
		'allowedExtensions' => 'bmp,gif,jpeg,jpg,png,pdf',
		'deniedExtensions' => '.php');
...
*/
$config['CheckDoubleExtension'] = false; /* !!!!!!!! DO NOT CHANGE THIS CONFIG VALUE !!!!!!! */
...
The * and ? wildcards are accepted.
*/
$config['HideFiles'] = Array("albumicon-default.png");
...
include_once "plugins/dxmedia/plugin.php";
include_once "plugins/imagedesc/plugin.php";
