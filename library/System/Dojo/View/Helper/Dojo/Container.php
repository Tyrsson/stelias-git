 ...
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 */
class System_Dojo_View_Helper_Dojo_Container
{
    /**
     ...
     * Set view object
     *
     * @param  System_Dojo_View_Interface $view
     * @return void
     */
     ...
     * Enable dojo
     *
     * @return System_Dojo_View_Helper_Dojo_Container
     */
    ...
    public function requireModule($modules)
    {
        if (!is_string($modules) && !is_array($modules)) {
            require_once 'System/Dojo/View/Exception.php';
            throw new System_Dojo_View_Exception('Invalid module name specified; must be a string or an array of strings');
        }
        ...
        foreach ($modules as $mod) {
            if (!preg_match('/^[a-z][a-z0-9._-]+$/i', $mod)) {
                //TODO:: Fix this regex check
                //require_once 'System/Dojo/View/Exception.php';
                //throw new System_Dojo_View_Exception(sprintf('Module name specified, "%s", contains invalid characters', (string) $mod));
            }
    ...
    public function addStylesheetModule($module)
    {
        if (!preg_match('/^[a-z0-9]+\.[a-z0-9_-]+(\.[a-z0-9_-]+)*$/i', $module)) {
            ...
            throw new System_Dojo_View_Exception('Invalid stylesheet module specified');
        }
     ...
     * @return System_Dojo_View_Helper_Dojo_Container|bool
     */
    ...
    public function onLoadCaptureStart()
    {
        if ($this->_captureLock) {
            require_once 'System/Dojo/View/Exception.php';
            throw new System_Dojo_View_Exception('Cannot nest onLoad captures');
        }
    ...
    public function addDijit($id, array $params)
    {
        if (array_key_exists($id, $this->_dijits)) {
            require_once 'System/Dojo/View/Exception.php';
            throw new System_Dojo_View_Exception(sprintf('Duplicate dijit with id "%s" already registered', $id));
        }
...
EOJ;
    ...
    public function javascriptCaptureStart()
    {
        if ($this->_captureLock) {
            require_once 'Zend/Dojo/View/Exception.php';
            throw new System_Dojo_View_Exception('Cannot nest captures');
        }
    ...
    public function __toString()
    {
        ...
        if (System_Dojo_View_Helper_Dojo::useDeclarative()) {
