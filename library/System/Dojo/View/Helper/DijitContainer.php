 ...
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
  */
abstract class System_Dojo_View_Helper_DijitContainer extends System_Dojo_View_Helper_Dijit
{
    /**
     * Capture locks
     * @var array
    ...
    public function captureStart($id, array $params = array(), array $attribs = array())
    {
        if (array_key_exists($id, $this->_captureLock)) {
            ...
            throw new System_Dojo_View_Exception(sprintf('Lock already exists for id "%s"', $id));
        }
    ...
    public function captureEnd($id)
    {
        if (!array_key_exists($id, $this->_captureLock)) {
            require_once 'Zend/Dojo/View/Exception.php';
            throw new System_Dojo_View_Exception(sprintf('No capture lock exists for id "%s"; nothing to capture', $id));
        }
