 ...
 */
class System_Form_Element_Thumbnail extends Zend_Form_Element_File
{
    /**
     * Plugin loader type
     */
    ...
    protected $thumbMaxHeight = 120;
    protected $thumbMaxWidth = 120;

    protected $thumbNamePrefix = null;

    //protected $appSettings;
    public function init()
    {
        //TODO:: Return thumbnail name to calling class within the file data, so that the thumbnail file name can be saved
    ...
    public function setOptions(array $options)
    {
        ...
        // set the width and height from an options array
        // height
        if(isset($options['thumbMaxHeight'])) {
            $this->setThumbMaxHeight ($options['thumbMaxHeight']);
            unset($options['thumbMaxHeight']);
        }
        elseif( !isset($options['thumbMaxHeight']) && isset($settings->thumbMaxHeight) && !empty($settings->thumbMaxHeight)) {
            $this->setThumbMaxHeight ($settings->thumbMaxHeight);
            unset($options['thumbMaxHeight']);
        }
        // width
        if(isset($options['thumbMaxWidth'])) {
            $this->setThumbMaxWidth ($options['thumbMaxWidth']);
            unset($options['thumbMaxWidth']);
        }
        elseif( !isset($options['thumbMaxWidth']) && isset($settings->thumbMaxWidth) && !empty($settings->thumbMaxWidth)) {
            $this->setThumbMaxWidth ($settings->thumbMaxWidth);
            unset($options['thumbMaxWidth']);
        }

        if($options['thumbNamePrefix'] !== null && $options['thumbNamePrefix'] !== '') {
            $this->setThumbNamePrefix ($options['thumbNamePrefix']);
            unset($options['thumbNamePrefix']);
        }
	...
	/**
     * @return the $thumbNamePrefix
     */
    public function getThumbNamePrefix ()
    {
        return $this->thumbNamePrefix;
    }

	/**
     * @param field_type $thumbNamePrefix
     */
    public function setThumbNamePrefix ($thumbNamePrefix)
    {
        $this->thumbNamePrefix = $thumbNamePrefix;
    }

	////////////////////////////////////////////
    /**
     * @return the $thumbMaxHeight
     */
...
###############################################################################################################################################
    ...
    public function receive()
    {
        ...
        if ($adapter->receive($this->getName())) {
            //Zend_Debug::dump($this->getFileInfo($value = null), 'System_Form_Element::getFileInfo($value = null)');
            $files = $this->getFileInfo($value = null);
            foreach($files as $file) {
                //Zend_Debug::dump($file);
                if($file['name'] !== '') {
                    $this->createThumb($file, $file['destination']);
                }
    ...
    public function createThumb($fileData, $destination)
    {
    	$basePath = $destination;
    	$destination = dirname($basePath) . DIRECTORY_SEPARATOR . 'thumbs';
        define('MAX_WIDTH', $this->thumbMaxWidth);
        define('MAX_HEIGHT', $this->thumbMaxHeight);
        // process the uploaded image
        if ($this->isReceived()) {
            if($fileData['tmp_name'] !== '') {
            	$original = $fileData['tmp_name'];
            ...
            $imagetypes = array('/\.gif$/' , '/\.jpg$/', '/\.JPG$/' , '/\.jpeg$/' , '/\.png$/');
            $name = preg_replace($imagetypes, '', $fileData['name']);
            //$thumb_name = 'thumb_'.$name;
            $filter = new System_Filter_ThumbFileNamePrefix();
            if($this->thumbNamePrefix !== null) {
                $filter->setPrefix($this->thumbNamePrefix);
            }
            $thumb_name = $filter->filter($name);
            //Zend_Debug::dump($thumb_name);
           // die();
            ...
            if (! $source) {
                throw new Zend_Application_Exception('Image source is corrupt.');
            } else {
                ...
                switch ($type) {
                	case 1:
                	    ...
                	    imagecopyresampled($thumb, $source, 0, 0, 0, 0, $thumb_width, $thumb_height, $width, $height);
                	    imagegif($thumb, $destination . DIRECTORY_SEPARATOR . $thumb_name . '.gif');

//                 	    if (function_exists('imagegif')) {
//                 	        $thumb_name = 'thumb.' . $name;
//                 	        $success = imagegif($thumb, $destination . DIRECTORY_SEPARATOR . $thumb_name . '.gif');
//                 	    } else {
//                 	        $thumb_name = 'thumb.' . $name;
//                 	        $success = imagejpeg($thumb, $destination . DIRECTORY_SEPARATOR . $name . '.jpg', 50);
//                 	    }
                	    break;
                	case 2:
                	    //$thumb_name = 'thumb.' . $name;
                	    $success = imagejpeg($thumb, $destination . DIRECTORY_SEPARATOR . $thumb_name . '.jpg', 100);
                	    break;
                	case 3:
                	    imagecolortransparent($thumb, imagecolorallocatealpha($thumb, 0, 0, 0, 127));
                	    imagealphablending($thumb, false);
                	    imagesavealpha($thumb, true);
                	    imagecopyresampled($thumb, $source, 0, 0, 0, 0, $thumb_width, $thumb_height, $width, $height);
                	    $success = imagepng($thumb, $destination . DIRECTORY_SEPARATOR . $thumb_name . '.png');
                	    break;
                }
                if ($success) {
                    ...
                    throw new System_Application_Exception('Application error, could not create thumbnail');
                }
                // remove the image resources from memory
                imagedestroy($source);
                imagedestroy($thumb);
            ...
            throw new Zend_Application_Exception('The file has not been received!');
        }
    }
