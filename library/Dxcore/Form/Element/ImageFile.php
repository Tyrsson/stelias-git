 ...
 */
class Dxcore_Form_Element_ImageFile extends Zend_Form_Element_File
{
    /**
     * Plugin loader type
     */
...
###############################################################################################################################################
    ...
    public function receive()
    {
        ...
        if ($adapter->receive($this->getName())) {
            //Zend_Debug::dump($this->getFileInfo($value = null), 'Dxcore_Form_Element::getFileInfo($value = null)');
            ...
            if (! $source) {
                ...
                switch ($type) {
                	case 1:
                	    ...
                	    imagecopyresampled($thumb, $source, 0, 0, 0, 0, $thumb_width, $thumb_height, $width, $height);
                	    imagegif($thumb, $destination . DIRECTORY_SEPARATOR . $name . '.gif');
...
//                 	    }
                	...
                	case 3:
                	    ...
                	    imagesavealpha($thumb, true);
                	    imagecopyresampled($thumb, $source, 0, 0, 0, 0, $thumb_width, $thumb_height, $width, $height);
                	    $success = imagepng($thumb, $destination . DIRECTORY_SEPARATOR . $name . '.png');    
                	    break;
                }
                if ($success) {
                    ...
                    throw new Dxcore_Application_Exception('Application error, could not create thumbnail');
                }
                // remove the image resources from memory
                imagedestroy($source);
                imagedestroy($thumb);
