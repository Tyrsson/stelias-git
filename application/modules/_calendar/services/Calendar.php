...
class Calendar_Service_Calendar
{
     ...
     * @param String $locale            
     */
    public function __construct ($date = null, $locale = "en_US", array $config = null)
    {
        $front = Zend_Controller_Front::getInstance();
        $this->view = $front->getParam('bootstrap')->getResource('view');
       // $view = $bootstrap->get
        //$this->jquery = new ZendX_JQuery_View_Helper_AjaxLink();
        $this->db = new Calendar_Model_Calendar();
        $this->db->init();
        $this->setDate($date, $locale);
        
        $this->reservedDates = array(
                5,
                7,
                9,
                10,
...
//         );
        ...
        $this->eventMonths = $this->db->getEventMonths();
        
        // this needs refacotred to return actual db query, is currently hardcoded for testing
        $this->events = $this->db->getEventDaysByMonth($this->getYear(),$this->getMonthName());
        if(!empty($this->events['days']))
        {
            $this->hasEvent = true;
            $this->eventDays = explode(',', $this->events['days']);
        }
        Zend_Debug::dump($this->events);
        
        $this->eventMonths['August']['eventdays'] = array(
                5,
                7,
                9,
                10
        ...
        $this->eventMonths['December']['eventdays'] = array(
                25
        );
        ...
        // $this->service = new Calendar_Service_Google($this->gCalStartMin,
        // $this->gCalStartMax);
        ...
        // foreach($gCal as $cal) {
        // // blank for the moment as we are not supporting multiple calendars
        // yet
        // }
        ...
        // this is here for any external config, config can also be set via
        // setOptions();
        if($config == null) {
            $this->setOptions(array('name' => 'default'));
        }
        self::init();
    }
    ...
    public function setOptions (array $options)
    {
        ...
        if (array_key_exists('name', $options)) {
            $this->setCalendarName($options['name']);
        }
    }
    ...
    public function getCalendarName() {
        if($this->calendarName == null || !isset($this->calendarName))
        {
            $this->calendarName = 'default';
        }
    ...
    {
        $this->_monthNames = Zend_Locale::getTranslationList('Month', 
                $this->_locale); // locale
                                                                                               // month
                                                                                               // list
                                                                                               // Zend_Debug::dump($this->_monthNames,
                                                                                               // 'initDateParams:
                                                                                               // $this->_monthNames');
        
        $this->_dayNames = Zend_Locale::getTranslationList('Day', 
                $this->_locale); // locale
                                                                                           // day
                                                                                           // list
                                                                                           // Zend_Debug::dump($this->_dayNames,
                                                                                           // 'initDateParams:
                                                                                           // $this->_dayNames');
                                                                                           // #####################################
        $this->setMonthsInRange(); // set locale valid dates
                                   // Zend_Debug::dump($this->setMonthsInRange(-1,
                                   // 12), 'initDateParams:
                                   // $this->setMonthsInRange()');
        
        $this->_numMonthDays = $date->get(Zend_Date::MONTH_DAYS); // num days in
                                                                  // locale
                                                                  // month
                                                                  // Zend_Debug::dump($this->_numMonthDays,
                                                                  // 'initDateParams:
                                                                  // $this->_numMonthDays');
        
        $this->setNextMonth($date); // set the next month
                                    // Zend_Debug::dump($this->setNextMonth($date),
                                    // 'initDateParams:
                                    // $this->setNextMonth($date)');
        
        $this->setPrevMonth($date); // set the previous month
                                    // Zend_Debug::dump($this->setPrevMonth($date),
                                    // 'initDateParams:
                                    // $this->setPrevMonth($date)');
        
        $this->_firstDayOfWeek = $date->get(Zend_Date::WEEKDAY_DIGIT); // first
                                                                       // day of
                                                                       // the
                                                                       // curr
                                                                       // month
                                                                       // Zend_Debug::dump($this->_firstDayOfWeek,
                                                                       // 'initDateParams:
                                                                       // $this->_firstDayOfWeek');
        
        $this->_numWeeks = ceil(
                ($this->getFirstDayOfWeek() + $this->getNumMonthDays()) / 7); // num
                                                                                                      // weeks
                                                                                                      // in
                                                                                                      // curr
                                                                                                      // month
                                                                                                          // Zend_Debug::dump($this->_numWeeks,
                                                                                                      // 'initDateParams:
                                                                                                      // $this->_numWeeks');
    }
    ...
    public function setMonthsInRange ($startOffset = -1, $endOffset = 12)
    {
        ...
        $this->_validDates[$startMonth->get("MMMM yyyy")] = $startMonth->get(
                "MMMM yyyy");
    ...
    public function setLocale ($locale = null)
    {
        try {
            ...
            $this->_log->log(
                    'Calendar Exception: ' . 'File: ' . $e->getFile() . 'Line: ' .
    ...
    public function getCalendarHeaderHtml ($arr = NULL)
    {
        ...
        if ($showPrevMonthLink) {
            ...
            if (! array_key_exists($t, $this->_validDates)) // check if the prev
                                                           // month in list of
                                                           // valid dates
                $pLinkClass = "id=\"prevMonth\" style=\"visibility: hidden;\"";
                // Original code -> $pLink = "<a $pLinkClass
            // href=\"?$selectBoxName=" . urlencode($t) .
            // "\">&lt;&nbsp;$t</a>\n";
            $pLink = "<a $pLinkClass href=\"/calendar?$selectBoxName=" .
                     urlencode($t) . "\">&lt;&nbsp;$t</a>\n";
        }
        if ($showNextMonthLink) {
            $t = $this->getNextMonthAsDateString();
            if (! array_key_exists($t, $this->_validDates)) // check if the next
                                                           // month in list of
                                                           // valid dates
                $nLinkClass = "id=\"nextMonth\" style=\"visibility: hidden;\"";
                // Original code -> $nLink = "<a $nLinkClass
            // href=\"?$selectBoxName=" . urlencode($t) .
            // "\">$t&nbsp;&gt;</a>\n";
            $nLink = "<a $nLinkClass href=\"calendar?$selectBoxName=" .
                     urlencode($t) . "\">$t&nbsp;&gt;</a>\n";
        }
        ...
        if ($selectBox) {
            ...
            $headDate .= $this->getValidDatesSelectBox(
                    array(
                            'selectedDateStr' => $this->getDateAsString(),
    ...
    public function getCalendarBodyHtml ($arr = NULL)
    {
        ...
        // this hits the current month, this should be the array key that
        // contains an array of event days for the iteration below
        // Jsmith
        ...
        for ($i = 0; $i < $this->getNumWeeks(); $i ++) {
            ...
            for ($j = 0; $j < 7; $j ++) {
                ...
                if($this->hasEvent) {
                    if (count($this->eventDays) > 0) {
                        if (in_array($calDayNum, $this->eventDays)) {
                            $class = "class = \"event\"";
                        }
                    }
                    if (count($this->eventDays) > 0) {
                        if ($showToday && $nowDate == $focusDate && $today == $calDayNum && $cellNum >= $this->getFirstDayOfWeek() &&
                                 in_array($today, $this->eventDays)) {
                            $class = "class = \"todayisevent\"";
                        }
                    }
                    $openLink = "<a href=\"/admin/calendar/manage-events\">";
                    //echo $openLink;
                    $closeLink = "</a>";
                }
                else{
                ...
                $html .= "<td $class>"; //<- original line and location
                
                
                if ($cellNum >= $this->getFirstDayOfWeek() && $cellNum < ($this->getNumMonthDays() + $this->getFirstDayOfWeek())) {
                    
                    if($this->hasEvent) {
                        //$html .= '<a href="/admin/calendar/manage-events?year='.$this->getYear().'&amp;month='.$this->getMonthName().'&amp;day='.$calDayNum.'">';
                        $html .= $this->view->ajaxLink($calDayNum, '/admin/calendar/manage-event?format=html&calName='.$this->getCalendarName().
                                                                                                                     '&year='.$this->getYear().
