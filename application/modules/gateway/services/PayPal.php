...
class Gateway_Service_PayPal
{
    ...
    public static function factory(Gateway_Service_PayPal_Data_AuthInfo $authInfo, $options = array(), $apiMode = null)
    {
        if (! is_null($apiMode) && ($apiMode == self::API_MODE_NVP || $apiMode == self::API_MODE_SOAP)) {
            throw new Gateway_Service_PayPal_Exception('$apiMode must be either "Nvp" or "Soap"');
        }
        
        if ($apiMode == self::API_MODE_SOAP) {
            throw new Gateway_Service_PayPal_Exception('Soap is not yet supported');
        }
        
        return new Gateway_Service_PayPal_Nvp($authInfo, $options);
    }
